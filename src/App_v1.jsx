...
function App() {
  const expensesListModified = expensesList.map((expense) => {
    return { ...expense, amount: Math.ceil(expense.amount) };
  });
  ...
  return (
    <div className={`${overlay ? "app-disabled" : "app"}`}>
      ...
      <button className="btn show" onClick={()=>setShowCharts(flag=>!flag)}>{showCharts?'Show Tables':'Show Charts'}</button>
      {expenses.length > 0 && (
        <div className="container">
         
            <Filter
              setFilteredExpenses={setFilteredExpenses}
              expenseList={expensesListModified}
            />
          {!showCharts?  <ExpenseList
              expenses={expenses}
    ...
    <Overlay onClick={handleFormClose}>
      ...
      <option value="Snacks">Snacks</option> */}
          ...
          <div className="form-btn">
            {" "}
            <Button>{expense ? "Edit" : "Add"}</Button>
          </div>
        </form>
      </div>
    </Overlay>
...
function ExpenseList({
  ...
  function onAddExpense(expense) {
    setExpenses(
      expenses.map((data) => (data?.id == expense?.id ? expense : data))
    );
  ...
  return (
    <>
      <table>
        <thead>
          <tr>
            <th onClick={() => onSort("amount", amountSort)}>
              Amount
              {amountSort === "none" && (
                <svg
            ...
            <th onClick={() => onSort("category", categorySort)}>
              Category
              {categorySort === "none" && (
                <svg
                  xmlns="http://www.w3.org/2000/svg"
            ...
            <th onClick={() => onSort("date", dateSort)}>
              Date
              {dateSort === "none" && (
                <svg
                  xmlns="http://www.w3.org/2000/svg"
      ...
      {editExpenseForm && (
        <AddExpenseForm
          ...
          expense={editExpense}
          handleFormClose={() => setEditExpenseForm(false)}
        />
      )}
    </>
  );
...
function Filter({ setFilteredExpenses, expenseList }) {
  ...
  function handleAmountApply() {
    ...
    if (minDate !== "" && maxDate !== "") {
      ...
      setFilteredExpenses((expenses) =>
        expenses.filter(
          (expense) =>
            new Date(expense.date) >= minTimestamp &&
            new Date(expense.date) <= maxTimestamp
        )
      );
    }
  }
  ...
  function handleAmountClear() {
    ...
    setFilteredExpenses(
      expenseList.filter((expense) => {
        ...
        const dateCondition =
          minDate !== "" && maxDate != ""
            ? new Date(expense.date) >= new Date(minDate) &&
              new Date(expense.date) <= new Date(maxDate)
            : true;
  ...
  function findDate(minDays, maxDays) {
    ...
    if (maxDays <= 7) {
      ...
      mindate = new Date(Date.UTC(mindate.getFullYear(), mindate.getMonth(), 1))
        .toISOString()
        .split("T")[0];
      maxdate = maxdate.toISOString().split("T")[0];
    } else if (maxDays <= 62) {
      mindate = new Date(
        Date.UTC(mindate.getFullYear(), mindate.getMonth() - 1, 1)
      )
        .toISOString()
        .split("T")[0];
      let currentMonthFirstDate = new Date(
        Date.UTC(maxdate.getFullYear(), maxdate.getMonth(), 1)
      );
  ...
  function handleCategoryChange(e) {
    let isCategoryPresent =
      filteredCategory.findIndex((value) => value === e.target.value) >= 0;
    if (isCategoryPresent) {
      setFilteredCategory((category) =>
        category.filter((value) => value !== e.target.value)
      );
    } else {
  ...
  return (
    <>
      <div className="filter">
        ...
        >
          <button
            className={`${
              amountApplyClicked ? " btn btn-filter-selected" : "btn"
            }`}
          >
            Amount
          </button>
              ...
              <fieldset>
                <div className="radio-amount">
                  <input
                    ...
                    name="amount"
                    value="0-50"
                    checked={minAmount === 0 && maxAmount === 50}
                    min={0}
                    max={50}
                ...
                <div className="radio-amount">
                  <input
                    ...
                    name="amount"
                    value="50-100"
                    checked={minAmount === 50 && maxAmount === 100}
                    min={50}
                    max={100}
                    onChange={handleAmountChange}
                  />
                ...
                <div className="radio-amount">
                  <input
                    ...
                    name="amount"
                    value="100-150"
                    checked={minAmount === 100 && maxAmount === 150}
                    min={100}
                    max={150}
                    onChange={handleAmountChange}
                  />
        ...
        >
          <button
            className={`${
              categoryApplyClicked ? "btn btn-filter-selected" : "btn"
            }`}
          >
            Category
          </button>
            ...
            <div className="category">
              ...
              <fieldset>
                {categoriesList.map((category2) => {
                  return (
                    <div className="checkbox" key={category2}>
                      <input
                        type="checkbox"
                        id={category2}
                        name="category"
    ...
    <label htmlFor='snacks'>Snacks</label>
              ...
              <div className=" action-btn">
                {
                  <button
                    className={`' btn ${
                      filteredCategory.length == 0 && !categoryApplyClicked
                        ? "btn-disabled"
                        : "btn-clear"
                    } `}
                    disabled={
                ...
                <button
                  className={` btn ${
                    filteredCategory.length == 0 ? "btn-disabled" : "btn-apply"
                  } `}
                  disabled={filteredCategory.length == 0}
        ...
        >
          <button
            className={`${
              dateApplyClicked ? "btn btn-filter-selected" : "btn"
            }`}
          >
            Date
          </button>
              ...
              <div className=" action-btn">
                {
                  <button
                    className={`' btn ${
                      minDate === "" || (maxDate === "" && !dateApplyClicked)
                        ? "btn-disabled"
                        : "btn-clear"
                    } `}
                    disabled={
